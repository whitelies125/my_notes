...
光栅化是根据图元的 pixel 覆盖范围， 将每个单独的图元分解为称为 fragment 片段的离散元素的过程。
...
最后一组数据是通过对片段顶点的数据值进行**插值**计算得出的。插值方式由输出这些值的着色器定义。
...
最常见的操作是用来实现物体透明度。
参考 LearnOpenGL 中，使用混合实现丢弃片段、半透明纹理：
